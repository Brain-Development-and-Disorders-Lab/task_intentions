...
describe("Compute", () => {
  ...
  describe("Submit Method", () => {
    ...
    test("should submit with test responses successfully", async () => {
      ...
      expect(mockWebRInstance.evalR).toHaveBeenCalledWith(expect.stringContaining("model_wrapper(fromJSON('["));
      expect(mockConsola.success).toHaveBeenCalledWith(expect.stringContaining("Compute complete after"));
  ...
  describe("Performance Scenarios", () => {
    ...
    test("should handle high hardware spec conditions", async () => {
      ...
      mockPerformance.now.mockReturnValueOnce(1000).mockReturnValueOnce(1050); // 50ms execution time
    ...
    test("should handle low hardware spec conditions", async () => {
      ...
      mockPerformance.now.mockReturnValueOnce(1000).mockReturnValueOnce(10000); // 9 second execution time
    ...
    test("should handle concurrent requests", async () => {
      ...
      const promises = Array.from({ length: 5 }, (_, i) =>
        compute.submit(
          [{ ID: `CONCURRENT_${i}`, Trial: 1, ppt1: 5, par1: 7, ppt2: 8, par2: 6, Ac: 1, Phase: 1 }],
          false
        )
      );
  ...
  describe("Error Handling and Edge Cases", () => {
    ...
    test("should handle null data", async () => {
      ...
      expect(mockWebRInstance.evalR).toHaveBeenCalledWith(expect.stringContaining("model_wrapper(fromJSON('null'))"));
      expect(result).toBeDefined();
    });
