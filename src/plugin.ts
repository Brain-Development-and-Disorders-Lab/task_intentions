 ...
 *
 * The plugin integrates with the `neurocog` cross-platform API and manages
...
// Utility functions
import { saveToLocalStorage, setCompleted } from "src/util";
...
jsPsych.plugins[Configuration.studyName] = (() => {
  ...
  plugin.trial = (displayElement: HTMLElement, trial: IntentionsNode) => {
    ...
    const dataframe: Dataframe = {
      ...
      correctGuess: NaN, // whether the participant guessed correctly
      trialDuration: NaN, // duration of the trial in ms
      setupDuration: NaN, // duration of the setup operation in ms
      operationDuration: NaN, // duration of the operation in ms
