...
const DASS: FC<Props.Screens.DASS> = (
  ...
  // Split questions into pages (10 questions on page 2, 11 on page 3)
  const questionsPage2 = questions.slice(0, 10);
  const questionsPage3 = questions.slice(10);
  ...
  // Render page 1 (DASS explanation)
  if (currentPage === 1) {
    return (
      <Box
        justify={"center"}
  ...
  // Render page 2 (first 10 questions)
  if (currentPage === 2) {
    return (
      <Box
        justify={"center"}
      ...
      >
        ...
        >
          {questionsPage2.map((question: string, index: number) => (
            <Box
              key={index}
              direction="row"
              align="center"
              ...
              background="light-1"
            >
              <Paragraph margin="none" size="medium" style={{ flex: 1 }}>
                {index + 1}. {question}
              </Paragraph>
  ...
  // Render page 3 (remaining 11 questions)
  return (
    <Box
      justify={"center"}
      align={"center"}
    ...
    >
                    <Heading level={3} margin="small" fill>
          DASS-21 Questions (11-21)
        </Heading>
        <Paragraph margin="small" size="small" textAlign="center" color="dark-4">
          Page 3 of 3
        </Paragraph>
      ...
      <Box
        ...
        style={{ maxHeight: "70vh", overflowY: "auto" }}
      >
        {questionsPage3.map((question: string, index: number) => (
          <Box
            key={index + 10}
            direction="row"
            align="center"
          ...
          >
            ...
            <RadioButtonGroup
              ...
              value={responses[index + 10] === -1 ? undefined : responses[index + 10]}
              onChange={event => updateResponse(index + 10, Number(event.target.value))}
            />
          </Box>
        ))}
      </Box>
